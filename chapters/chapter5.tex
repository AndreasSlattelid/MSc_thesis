...
In typical futures contracts, $X$ could for instance be oil barrels, however, in our situation, $X$ represents an underlying interest rate.  For interest rates, the typical settlement would be a cash settlement.
\\~\\ 
\textbf{Reasons to enter futures contract:}
\begin{itemize}
    \item Speculation: By trading futures, one can make money by differences in quotes.  
    \item Hedging: One could hedge against higher/lower interest rates. For instance, someone with paying a floating rate might buy futures to lock in a future interest rate. 
\end{itemize}

We will be interested in SOFR futures, such futures can be found at CME (Chicago Mercantile Exchange) \nomenclature{CME}{Chicago Mercantile Exchange}. For instance, a 3M-SOFR future with settlement Jun 23 quoted at $96.6$ would mean that the implied $3M$-SOFR rate over the period March 23 -June 23 would be: 
\[
(100-96.6)\% = 3.4\% 
\] 
For further specifications for the 3M-SOFR futures, one can consult \cite{cmegroup-sofr-futures}. 
...
\begin{align*}
H(a_{t}) &= \E_{Q}\left[(X^{3M_{A}}(S,T))^{2}|\F_{t}\right] -2a_{t}f^{3M}(t,S,T) +
a_{t}^{2}[f^{3M}(t,S,T)]^{2}
\end{align*}
Taking the derivative w.r.t. $a_{t}$ yields: 
\begin{align*}
\frac{d}{da_{t}}H(a_{t}) &= -2f^{3M}(t,S,T) + 2a_{t}[f^{3M}(t,S,T)]^{2}  
\end{align*} 

Now as $\frac{d^{2}}{da_{t}^{2}}H(a_{t}) = 2[f^{3M}(t,S,T)]^{2} > 0$, we have that the minimum is obtained by setting $\frac{d}{da_{t}}H(a_{t}) = 0$:
\begin{align*}
\frac{d}{da_{t}}H(a_{t}) &= 0 \\ 
&\Downarrow \\ 
a_{t} &= \frac{
...
We now denote: 
\begin{align*}
X^{3M_{A}}(S,T) &= \frac{1}{T-S}\int_{S}^{T}r_{u}du = \frac{1}{T-S}Z \\ 
...
\begin{align*}
Er(t) :&= X^{3M_{A}}(S,T) - \hat{a}_{t}f^{3M}(t,S,T) \\ 
...
We work with the Vasicek model: 
\begin{align*}
dr(t) &= \alpha[m-r(t)]dt + \sigma dW^{Q}(t)    
\end{align*}

In the simulation, we use the closed expression for $\int_{S}^{T}r(u)du$  as described in Equation \ref{eq: Vasicek_closed_expr_integral} on Page \pageref{eq: Vasicek_closed_expr_integral}. We fix $t=0$, and will therefore study $Er(0)$: 

\begin{figure}[htp]
    \centering
    \includegraphics[width=12cm]{figures/SOFR/Vasicek_path.png}
    \caption{Path of $r = (r(u))_{u \in 0,T]}$}
    \label{fig: Vasicek_path}
\end{figure}

\newpage 

We ran the simulation in Julia and got the following:

\begin{figure}[htp]
    \centering
    \includegraphics[width=12cm]{figures/SOFR/Error_hist.png}
    \caption{Simulation of $Er(0)$}
    \label{fig: Error_simulation_Julia}
\end{figure}

\newpage 

\subsection{Hedging three-month arithmetic with three 1M-SOFR futures}
...
Fix $t$, and denote $(a_{t}, b_{t}, c_{t}) = (x_{1}, x_{2}, x_{3}) = \mathbf{x}$
...
\begin{align*}
\partial_{x_{1}}G(\mathbf{x}) &= 
-2\E_{Q}[X^{3M_{A}}(S,T)|\F_{t}]f^{1M}(t,S, T_{1M}) 
+ 2x_{1}[f^{1M}(t,S,T_{1M})]^{2} \\ 
&+ 2f^{1M}(t,S,T_{1M})\left[
x_{2}f^{1M}(t,T_{1M}, T_{2M}) + x_{3}f^{1M}(t,T_{2M}, T)
\right] \\ 
\\
\partial_{x_{2}}G(\mathbf{x}) &=
-2\E_{Q}[X^{3M_{A}}(S,T)|\F_{t}]f^{1M}(t,T_{1M}, T_{2M})
+ 2x_{1}f^{1M}(t,S,T_{1M})f^{1M}(t,T_{1M}, T_{2M}) \\ 
&+ 2x_{2}[f^{1M}(t,T_{1M}, T_{2M})]^{2} 
+ 2x_{3}f^{1M}(t,T_{1M}, T_{2M})f^{1M}(t,T_{2M}, T) \\ 
\\ 
\partial_{x_{3}}G(\mathbf{x}) &=
-2\E_{Q}[X^{3M_{A}}(S,T)|\F_{t}]f^{1M}(t,T_{2M}, T) 
+ 2x_{1}f^{1M}(t,S,T_{1M})f^{1M}(t,T_{2M}, T) \\ 
&+ 2x_{2}f^{1M}(t,T_{1M}, T_{2M})f^{1M}(t,T_{2M}, T)
+ 2x_{3}[f^{1M}(t,T_{2M}, T)]^{2}
\end{align*}

\newpage 

We also find the Hessian matrix $H(G)$ of $G$, this matrix will in our case be $3\times 3$ with entries: 
\begin{align*}
[H(G)]_{i,j} &= 
\frac{
\partial^{2}G
