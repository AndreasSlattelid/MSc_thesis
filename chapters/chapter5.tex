...
We see that the term SOFR rate is just a simple forward rate evaluated at time $t=S$ (See Definition \ref{def: Simple_forward_rate}, i.e $R^{F}(S,T) = F(S,S,T)$). 

\begin{proposition}[\textbf{\cite{Skov_2020}}]
We have the following relationship between the forward looking term SOFR rate $R^{F}(S,T)$ and the backward-looking SOFR rate $R^{B}(S,T)$: 
\begin{align*}
...
\begin{proof}
TO BE HONEST I DONT FULLY UNDERSTAND, WOULD LIKE SOME HELP. 
\end{proof}


As mentioned earlier we have that SOFR is backward looking, and cannot give any indications beyond 24-hours. However the CME group publishes daily a set of forward looking interest rate estimates, called CME Term SOFR Reference Rates Benchmarks. They use SOFR futures for calculating the term rates \cite{CMEGroup}.

The following tenors are published:
\begin{itemize}[leftmargin=*]
    \item 1M
    \item 3M
    \item 6M
