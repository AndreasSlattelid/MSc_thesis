...
\begin{align*}
\argmin\limits_{(a_{t}, b_{t}, c_{t})\in \R^{3}_{0}}\E_{Q}\left[
\left(
X^{3M_{A}}(S,T)- \left[
a_{t}f^{1M}(t,S,T_{1M}) + 
...
\newpage 
In order to obtain a minimum we will be interested in:
\begin{align*}
\nabla G(\mathbf{x}) &= 
\left(
...
Thus, the Hessian matrix looks like the following:
\begin{align*}
H(G) &= 
\begingroup
